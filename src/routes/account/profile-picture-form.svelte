<script lang="ts">
  ...
  import { profilePictureFormSchema } from './profile-picture-form-schema';
  ...
  const profilePictureForm = superForm(data.profilePictureForm, {
    validators: zod4Client(profilePictureFormSchema),
    onSubmit: () => {
      signoutError = false;
    },
    onError: () => {
  ...
  const { enhance, submitting } = profilePictureForm;
</script>
